...
class TestOps(unittest.TestCase, metaclass=ParameterizedTestMeta):
    ...
    PARAMS = {
        ...
        #                          depending on the other operand, and the
        #                          difference matters:
        ...
        #                              propagate_layouts allows it, correctly. The
        #                              compare is computed CORRECTLY too. But the
        #                              result carries the staggered EA, and the D2H
        #                              copy-out ignores element_arrangement and
        #                              reads the buffer as STANDARD, so the mask
        #                              comes back PERMUTED with no error raised.
        #                              These two cases therefore xfail on a NUMERIC
        #                              mismatch, not a compile error -- that is
        #                              deliberate: they pin the data corruption
        #                              itself.
        #                          The second reason is a pre-existing defect in the
        #                          copy-out path, NOT in this lowering, and not
        #                          specific to comparisons: plain add/mul on the
        #                          same operands stagger identically through stock
        #                          Inductor promotion, and so does a bare x.float()
        #                          returned directly -- hence .cpu().float() and
        #                          never .float().cpu(). A staggered EA is legit on
        #                          device: it is stamped on the live tensor and a
        #                          FOLLOWING graph consumes it correctly, so this
        #                          must be fixed at the readback (restickify, or the
        #                          reverse conversion), not by refusing the value.
        #                          Once copy-out honours the EA, the two broadcaster
        #                          cases should XPASS.
        #
        ...
        #                          question never comes up because no stick
        #                          reordering is emitted; the compare itself then
        #                          runs on device in fp16. It passes, but the green
        #                          means "correct via a host round-trip", NOT
        #                          "supported on device" -- do not read it as the
        #                          narrowing direction being cheap.
        #
        ...
        ("test_cmp_mixed_dtype", "test_cmp_mixed_dtype_cpu"): {
            ...
            "param_sets": {
                ...
                # fp16 vs a broadcaster at the stick dim: propagate_layouts.py
                # allows this combination (correctly -- a broadcaster has no stick
                # layout to disagree with) and the compare is computed correctly,
                # but the staggered result reads back PERMUTED, so these two xfail
                # on the numeric mismatch itself. They should XPASS once copy-out
                # honours element_arrangement.
                "fp16_fp32_bcast_stick": (
                    torch.ceil(cached_randn((4, 64), abs=True, scale=10.0)).to(
                        dtype=torch.float16
                    ),
                ...
                # int32 -> fp32 (int32tofp32) is 4B -> 4B: EA unchanged, all on
                # device -- the one mixed-dtype pair that is genuinely supported
                "int32_fp32_1d256": (
                    torch.randint(0, 100, (256,), dtype=torch.int32),
                    torch.ceil(
