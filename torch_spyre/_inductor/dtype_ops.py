...
#
# IEEE_INT32 is included because aten.le with INT_TO_FLOAT promotion casts
# int32 inputs to fp32 *inside* the lowering (via transform_args), so the
# native comparison runs in fp32 and the bool result is physically fp32-width.
# propagate_layouts sees the original int32 input layouts when inferring the
# bool output format, so it must map IEEE_INT32 → float32 here.
