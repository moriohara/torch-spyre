...
def _single_arg_op_layout(
    ...
    if isinstance(data, Reduction):
        ...
        # Determine output EA based on whether reduction is on stick dimension
        # If stick dimension is reduced (out_stick_dim is None or -1), EA becomes STANDARD
        # Otherwise, preserve input's EA
        if out_stick_dim is None or out_stick_dim == -1:
            # Stick dimension was reduced, normalize to STANDARD
            output_ea = ElementArrangement.STANDARD
            print(f"[EA_DEBUG] Reduction on stick dim: input_ea={stl.element_arrangement} -> output_ea=STANDARD")
            logger.info(f"[EA_DEBUG] Reduction on stick dim: input_ea={stl.element_arrangement} -> output_ea=STANDARD")
        else:
            # Stick dimension survives, preserve input EA
            output_ea = stl.element_arrangement
    ...
    print(f"[EA_DEBUG] _single_arg_op_layout: aten_op={aten_op}")
    match aten_op:
        ...
        case prims.convert_element_type.default:
            ...
            #
            # ElementArrangement handling for FP16↔FP32 conversions:
            # - FP16→FP32: Check input's ElementArrangement
            #   * If input has STANDARD: Set DL16_TO_FP32 (forward conversion creates non-sequential layout)
            #   * If input has DL16_TO_FP32: Already in DL16_TO_FP32, keep it
            # - FP32→FP16: Always use STANDARD (FP32_TO_DL16 is disabled)
            #   * If input has DL16_TO_FP32: Set STANDARD (reverse conversion normalizes layout)
            ...
            # Get input's ElementArrangement
            input_ea = stl.element_arrangement
            
            print(f"[EA_DEBUG] convert_element_type: {in_layout.dtype} -> {output.dtype}, input_ea={input_ea}")
            logger.info(f"[EA_DEBUG] convert_element_type: {in_layout.dtype} -> {output.dtype}, input_ea={input_ea}")

            # Determine output ElementArrangement based on dtype conversion and input EA
            if in_layout.dtype == torch.float16 and output.dtype == torch.float32:
                # FP16 → FP32: Check input's ElementArrangement
                # Forward conversion: input is native FP16 (STANDARD), converting to FP32
                # Set DL16_TO_FP32 for non-sequential layout
                fmt = ElementArrangement.DL16_TO_FP32
                print(f"[EA_DEBUG]   FP16->FP32: input={input_ea}, output=DL16_TO_FP32")
                logger.info(f"[EA_DEBUG]   FP16->FP32: input={input_ea}, output=DL16_TO_FP32")
            elif in_layout.dtype == torch.float32 and output.dtype == torch.float16:
                # FP32 → FP16: Always use STANDARD (FP32_TO_DL16 is disabled)
                fmt = ElementArrangement.STANDARD # TODO: Add FP32_TO_DL16
                print(f"[EA_DEBUG]   FP32->FP16: input={input_ea}, output=STANDARD")
                logger.info(f"[EA_DEBUG]   FP32->FP16: input={input_ea}, output=STANDARD")
            else:
                # All other conversions: use STANDARD
                fmt = ElementArrangement.STANDARD
                print(f"[EA_DEBUG]   Other conversion: output=STANDARD")
                logger.info(f"[EA_DEBUG]   Other conversion: output=STANDARD")
        ...
        case _:
            ...
            if (
                ...
                c_size = [concretize_expr(s) for s in output.size]
                c_stride = [concretize_expr(s) for s in output.stride]
                # When access patterns change, preserve EA only if element size is the same
                # (per TODO comment about preserving stride_map when elems_per_stick doesn't change)
...
def _multi_arg_pointwise_layouts(
    ...
    aten_op = origin_node.target if origin_node else None
    print(f"[EA_DEBUG] _multi_arg_pointwise_layouts: aten_op={aten_op}")
    stick_exprs = {
    ...
    for stick_expr in sorted(stick_exprs, key=iter_var_id) if stick_exprs else [None]:
        if can_use_same_layout:
            ...
            # All inputs have same layout, preserve EA
            output_ea = template_stl.element_arrangement
            print(f"[EA_DEBUG] multi_arg_pointwise (can_use_same_layout): preserving EA={output_ea}")
            logger.info(f"[EA_DEBUG] multi_arg_pointwise (can_use_same_layout): preserving EA={output_ea}")
            stl = SpyreTensorLayout(
            ...
            if len(input_eas) == 1:
                ...
                print(f"[EA_DEBUG] multi_arg_pointwise: all inputs have EA={output_ea}, preserving")
                logger.info(f"[EA_DEBUG] multi_arg_pointwise: all inputs have EA={output_ea}, preserving")
            elif ElementArrangement.DL16_TO_FP32 in input_eas and ElementArrangement.STANDARD in input_eas:
                # Mixed EA: check if STANDARD input is broadcast at stick dimension
                # For now, assume broadcast is valid and use DL16_TO_FP32
                # TODO: Add proper broadcast validation
                output_ea = ElementArrangement.DL16_TO_FP32
                print(f"[EA_DEBUG] multi_arg_pointwise: mixed EA with broadcast, using DL16_TO_FP32")
                logger.info(f"[EA_DEBUG] multi_arg_pointwise: mixed EA with broadcast, using DL16_TO_FP32")
            else:
                # Other mixed EA cases default to STANDARD
                output_ea = ElementArrangement.STANDARD
                print(f"[EA_DEBUG] multi_arg_pointwise: mixed EA {input_eas}, defaulting to STANDARD")
                logger.info(f"[EA_DEBUG] multi_arg_pointwise: mixed EA {input_eas}, defaulting to STANDARD")
            
            stl = SpyreTensorLayout(c_size, c_stride, output.dtype, dim_order, output_ea)
        results.append(stl)
